...
$(document).ready(function(){
	...
	function getAllObject(pageNo,pageSize,userCode) {
		$("#tBody").html("");
		$.post(
				"user/jump/order/getAllNoYsOrderByUser.do",
				{
					"pageNo":pageNo,
					"pageSize":pageSize,
					"userCode":userCode,
				...
				function (data) {
					...
					$(data.oList).each(function(i,n){
						...
						html += '<td ysStatus='+n.ysStatus+'>'+str+'</td>';
						if(n.ysStatus != 1){
							html += '<td class="id"><input class="del" type="button" objectCode="'+n.objectCode+'" orderCode="'+n.orderCode+'" value="点击收货"></td>';
						}else{
							html += '<td class="id"><input class="ref" type="button" objectCode="'+n.objectCode+'" orderCode="'+n.orderCode+'" value="拒绝收货"></td>';
						}
						html += '</tr>';
					});
					$("#tBody").html(html);
	...
	$("#tBody").on("click","td input[class='del']",{text:"确定收货吗？"},function(event){
		var orderCode = $(this).attr("orderCode");
		if(confirm(event.data.text)){
			$.post(
				"user/jump/order/yansuo.do",
				{
					"orderCode":orderCode,
					"userCode":userCode
				},
