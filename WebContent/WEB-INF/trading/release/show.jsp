...
<script type="text/javascript">
	 $(function() {
		...
		getAllObject(codeValue,pageNo,pageSize);
		function getAllObject(codeValue,pageNo,pageSize) {
			$("#ulBody").html("");
			$.post(
					"object/toObjectListtwo.do",
					{
						"codeValue":codeValue,
						"pageNo":pageNo,
						"pageSize":pageSize,
					...
					function (data) {
						...
						$(data.tList).each(function(i,n){
							html += '<li class="goods-panel">';
							html += '<a class="a" href="${pageContext.request.contextPath }/jump/toObject.do?objectCode='+n.objectCode+'" display="block">';
							html +=	'<img src="${pageContext.request.contextPath }/'+n.pictureDescri+'" width="200" height="200" title="点击查看物品详细信息" /><br/>';
							html +=	'<span>'+n.objectName+'</span><br/>';
							html +=	'<span>'+n.textDescri+'</span><br/>';
							html +=	'<span class="goods-price--grey"><span>售价:<i>￥</i>'+n.salePrice+'元</span></span><br/>';
							html += '</a>';
							html += '</li>';
						});
						$("#ulBody").html(html);
						...
						$("#mypagination").bs_pagination({
							  onChangePage: function(event, data) {
								  getAllObject(codeValue,data.currentPage,data.rowsPerPage);
							  }
							});
					});
		}
		...
		var MsgUserName =  $("#userMsgUserName").val();
...
<input type="text" hidden="hidden"  id="codeValue" value="${currentCode}"/>

<div style="position: absolute;top: 60px;height:150px; left: 1%;width: 30%;text-align: left;">
	<div style="position:absolute;top:20%; width: 40%;text-align: center;">
		<c:if test="${currentCode == objectType.objectCode }">
			<a class="getObjectCode" value="${objectType.objectCode}" href="${pageContext.request.contextPath}/jump/toObjectList.do?objectCode=${objectType.objectCode}">
			<span style="font-size: 30px;color: red;">${objectType.objectName }</span>
			</a>
		</c:if>
		<c:if test="${currentCode != objectType.objectCode }">
			<a class="getObjectCode" value="${objectType.objectCode}" href="${pageContext.request.contextPath}/jump/toObjectList.do?objectCode=${objectType.objectCode}">
			<span style="font-size: 30px;">${objectType.objectName }</span>
		</a>
		</c:if>
	</div>
	<div style="position: relative;top:20px;left: 41%;text-align: left;">
		<c:if test="${not empty pList }">
			<c:forEach items="${pList }" var="p" varStatus="vs">
				<c:if test="${currentCode == p.parObjectTypeCode }">
					<a class="getParObjectCode" value="${p.parObjectTypeCode }" href="${pageContext.request.contextPath}/jump/toObjectList.do?parObjectTypeCode=${p.parObjectTypeCode}">
					<span style="font-size: 25px; top:20px;color: red;">${p.parObjectTypeName }</span>
				</a>&nbsp;&nbsp;&nbsp;&nbsp;
				</c:if>
				<c:if test="${currentCode != p.parObjectTypeCode }">
					<a class="getParObjectCode" value="${p.parObjectTypeCode }" href="${pageContext.request.contextPath}/jump/toObjectList.do?parObjectTypeCode=${p.parObjectTypeCode}">
					<span style="font-size: 20px; top:20px;">${p.parObjectTypeName }</span>
				</a>&nbsp;&nbsp;&nbsp;&nbsp;
				</c:if>
			</c:forEach>
		</c:if>
...
<div style="position: absolute;top:60px;left: 30%;width: 69%;height: 150px;text-align: left;">
	<div style="position:absolute;">
		<h4>物品类型</h4>
		<c:if test="${ not empty allObjectTypeList }">
			<c:forEach items="${allObjectTypeList }" var="otl" varStatus="vs">
				<c:if test="${currentCode == otl.objectCode }">
					<a class="getObjectCode" value="${otl.objectCode }" href="${pageContext.request.contextPath}/jump/toObjectList.do?objectCode=${otl.objectCode}">
						<span style="font-size: 30px;color: red;">${otl.objectName }</span>
					</a>
				</c:if>
				<c:if test="${currentCode != otl.objectCode }">
					<a class="getObjectCode" value="${otl.objectCode }" href="${pageContext.request.contextPath}/jump/toObjectList.do?objectCode=${otl.objectCode}">
						<span style="font-size: 25px;">${otl.objectName }</span>
					</a>
				</c:if>
			</c:forEach>
		</c:if>
		<h4>物品分类</h4>
		<c:if test="${not empty parObjectTypeList }">
			<c:forEach items="${parObjectTypeList }" var="potl" varStatus="vs">
				<c:if test="${currentCode == potl.parObjectTypeCode }">
					<a class="getParObjectCode" value="${potl.parObjectTypeCode }" href="${pageContext.request.contextPath}/jump/toObjectList.do?parObjectTypeCode=${potl.parObjectTypeCode}">
						<span style="font-size: 25px;color: red;">${potl.parObjectTypeName }</span>
					</a>&nbsp;&nbsp;&nbsp;&nbsp;
				</c:if>
				<c:if test="${currentCode != potl.parObjectTypeCode }">
					<a class="getParObjectCode" value="${potl.parObjectTypeCode }" href="${pageContext.request.contextPath}/jump/toObjectList.do?parObjectTypeCode=${potl.parObjectTypeCode}">
						<span style="font-size: 20px;">${potl.parObjectTypeName }</span>
					</a>&nbsp;&nbsp;&nbsp;&nbsp;
				</c:if>
			</c:forEach>
		</c:if>
...
<div style="position: relative;top:205px; left: 1%;width: 99%;">
	<ul style="text-align: center;" id="ulBody">
		...
		<div style="text-align: center;"><div id="mypagination"></div></div>
</div>
